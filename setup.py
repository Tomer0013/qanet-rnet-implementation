...
def download_url(url: str, output_path: str, show_progress: bool = True) -> None:
...
def url_to_data_path(url: str) -> str:
...
def process_file(filename: str, data_type: str, word_counter: dict, char_counter: dict) -> tuple[list[dict], dict]:
...
def get_embedding(counter: Counter, data_type: str, limit: int = -1,
                  emb_file: str = None, vec_size: int = None, num_vectors: int = None) -> tuple[list, dict]:
...
def build_features(args: argparse.Namespace, examples: list[dict],
                   data_type: str, out_file: str, word2idx_dict: dict, char2idx_dict: dict) -> dict:
    para_limit = args.para_limit
    ques_limit = args.ques_limit
    ans_limit = args.ans_limit
    char_limit = args.char_limit

    def drop_example(ex: dict) -> bool:
    ...
    for n, example in tqdm(enumerate(examples)):
        ...
        def _get_word(word: str) -> int:
            for each in (word, word.lower(), word.capitalize(), word.upper()):
                if each in word2idx_dict:
                    return word2idx_dict[each]
            return 1

        def _get_char(char: str) -> int:
...
def save(filename: str, obj: Any, message: str = None) -> None:
